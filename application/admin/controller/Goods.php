...
class Goods extends AdminBase {
	...
	public function goodsAdd() {
		...
		if (!empty($_POST)) {
			...
			$data['stock'] = isset($_POST['stock']) ? $_POST['stock'] : 0;
			if(isset($_POST['specs'])){
			    ...
			    if(!empty($specs)) $data['specs'] = json_encode($specs);
            }
			...
			$data['jifen'] = isset($_POST['jifen']) ? $_POST['jifen'] : 0;
            $data['coin'] = isset($_POST['coin']) ? $_POST['coin'] : 0;
			$model = new GoodsModel();
			if ($id) {
				$data['update_time'] = time();//date('Y-m-d H:i:s');
				$res = $model->saveGoods($data, ['id' => $id]);
				$this->addlog('修改商品id:' . $id);
			} else {
				$data['create_time'] = time();
				$res = $model->insert($data);
				$this->addlog('新增商品' . $data['goods_name']);
			}
