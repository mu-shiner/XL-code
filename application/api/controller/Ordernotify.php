...
{   
    ...
    {   
        ...
        if($data['total_amount'] !=$orderinfo['order_price']) die('success');
        $time = time();
        $arr = [];
        // $lock = new RedisLock();
        // //获取锁（2秒过期）
    ...
    //         if($res == -1)
		  ...
		  //  }
            ...
            //redis存用户拼团列表
            $redis->hMset('user_'.$orderinfo['group_id'].'_'.$orderinfo['users_id'], $data);
            //redis存用户拼团id(方便定时任务取redis出价信息)
            $redis->rPush('group_join_list', 'user_'.$orderinfo['group_id'].'_'.$orderinfo['users_id']);
        ...
        {   
            if($data['state'] == 3)
            {
                ...
                if($orderinfo['type'] == 1)
        		{
        		    $filed = 'point';
        		    //返还提现额度
        		    $users->UsersNumInc(['users_id'=>$orderinfo['users_id']], 'point_withdraw_amount', $orderinfo['withdraw_price']);
        		}
        		...
        		elseif($orderinfo['type'] == 3)
        		{
        		    $filed = 'partner_money';
        		    //返还提现额度
        		    $users->UsersNumInc(['users_id'=>$orderinfo['users_id']], 'partner_withdraw_amount', $orderinfo['withdraw_price']);
        		}
                //返还余额
    ...
    {   
        ...
        try {
            ...
            if($orderinfo['type'] == 1)
            {
                $model->saveData(['is_partner'=>1,'partner_time'=>$time,'partner_pay'=>1,'direct_week_num'=>0],['users_id'=>$orderinfo['users_id']]);
                $bill = new UsersBill();
                $arr1 = [];
                ...
                if($info['parent_id'])
                {
                    ...
                    if($con['award'])
                    {
                        $this->partnerMoneyPay($info['parent_id'],$con['award'],$orderinfo['order_no']);
                    }
    ...
    //开通合伙人给上级返佣(若推荐人不是市场合伙人，推荐奖励1000元将按推荐关系一直往上查找，奖励给到关系最近的市场合伙人)
    public function partnerMoneyPay($users_id,$award,$orderno)
    {   
        $model = new Users();
        $parent = $model->getInfo(['users_id'=>$users_id],'username,parent_id,partner_money,is_partner');
        if($parent['is_partner'] == 1)
        {
            $arr = [];
            ...
            $arr['current_money'] = $parent['partner_money'];
            $arr['create_time'] = time();
            //存明细
            $bill = new UsersBill();
            $bill->addUserBill($arr);
            $model->UsersNumInc(['users_id'=>$users_id],'partner_money',$award);
        }
        else
        {   
            if($parent['parent_id'])
            {
                $this->partnerMoneyPay($parent['parent_id'],$award,$orderno);
            }
