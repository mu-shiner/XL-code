...
{   
   ...
   /**
    ...
    {   
        ...
        if($res['trade_status'] != 0)
        {
            return $this->error_req(100, '核销券转让中');
        }
        $users = new Users();
        $userinfo = $users->getInfo(['users_id'=>$res['users_id']],'status');
    ...
    {   
        ...
        $shop = new Shop();
        $shop_info = $shop->getInfo(['shop_id'=>$this->shop_id],'shop_id,shop_name,shop_status,logo,check_type,settlement_price,account,service_id');
        $id = $this->params['login_id'];
        $admin = new Admin();
        $admininfo = $admin->getInfo(['id'=>$id],'username,status');
        $verify_code = isset($this->params['verify_code']) ? $this->params['verify_code'] : '';
        $verify_model = new VerifyModel();
        ...
        try {
            ...
            $shop->ShopNumInc(['shop_id'=>$this->shop_id],'all_account',$shop_info['settlement_price']);
            //所属服务中心增加结算余额
